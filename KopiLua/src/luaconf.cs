...
//#endif
		...
		*/
		#if LUA_USE_APICHECK
			public static void luai_apicheck(lua_State L, bool o)	{Lua.Assert(o);}
			public static void luai_apicheck(lua_State L, int o) {Lua.Assert(o != 0);}
		#else
		...
		*/
		...
		public class CharPtr
		{
			...
			public static int operator -(CharPtr ptr1, CharPtr ptr2) {
				Lua.Assert(ptr1.chars == ptr2.chars); return ptr1.index - ptr2.index; }
		...
		{
			//Lua.Assert(false, "clearerr not implemented yet - mjf");
			return 0;
		}

		public static int setvbuf(Stream stream, CharPtr buffer, int mode, uint size)
		{
			Lua.Assert(false, "setvbuf not implemented yet - mjf");
			return 0;
		}
		...
		public static int GetUnmanagedSize(Type t)
		{
			if (t == typeof(global_State))
				...
				return 4;			
			Lua.Assert(false, "Trying to get unknown sized of unmanaged type " + t.ToString());
			return 0;
		}
	}
}
