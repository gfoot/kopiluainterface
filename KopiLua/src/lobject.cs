	...
	public partial class Lua
	{
		...
		*/
		...
		public class lua_TValue : ArrayElement
		{
			...
			{
				Lua.Assert(value.values == array);
				return value.index;
			}

			public static int operator -(lua_TValue a, lua_TValue b)
			{
				Lua.Assert(a.values == b.values);
		...
		*/
		...
		public class Node : ArrayElement
		{
			...
			{
				Lua.Assert(n1.values == n2.values);
			...
			public static bool operator >(Node n1, Node n2) { Lua.Assert(n1.values == n2.values); return n1.index > n2.index; }
			public static bool operator >=(Node n1, Node n2) { Lua.Assert(n1.values == n2.values); return n1.index >= n2.index; }
			public static bool operator <(Node n1, Node n2) { Lua.Assert(n1.values == n2.values); return n1.index < n2.index; }
			public static bool operator <=(Node n1, Node n2) { Lua.Assert(n1.values == n2.values); return n1.index <= n2.index; }
